...
class VocalRealismAdapter(nn.Module):
    ...
    def __init__(
        ...
        if max_f0_cents < 0 or max_volume_db < 0:
            raise ValueError("realism residual bounds must be non-negative")

        self.max_f0_cents = float(max_f0_cents)
        self.max_volume_db = float(max_volume_db)

        self.unit_norm = nn.LayerNorm(n_unit)
        self.input_proj = nn.Linear(n_unit + 2, hidden_channels)
    ...
    def forward(
        ...
        if units.ndim != 3 or f0.ndim != 3 or volume.ndim != 3:
            raise ValueError("units, f0 and volume must be [B, T, C] tensors")
        if units.shape[:2] != f0.shape[:2] or units.shape[:2] != volume.shape[:2]:
            raise ValueError("units, f0 and volume must share batch/frame dimensions")
        ...
        volume_gain = torch.pow(10.0, delta_volume_db / 20.0)
        volume_out = torch.clamp(volume * volume_gain, min=0.0)

        diagnostics = {
            "delta_f0_cents": delta_cents,
            "delta_volume_db": delta_volume_db,
        }
        return f0_out, volume_out, diagnostics
